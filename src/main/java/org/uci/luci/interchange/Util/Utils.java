...
public class Utils {
	...
	public static List<Node> findNodesWithAngleBetweenClosestTo180(
			Node centerNode, List<Node> nodes) {
		...
		for (int i = 0; i < nodes.size(); i++) {
			for (int ii = 0; ii < nodes.size(); ii++) {
				...
				double angle = Utils.angleBetween2Lines(centerNode.lon,
						centerNode.lat, node1.lon, node1.lat, centerNode.lon,
						centerNode.lat, node2.lon, node2.lat);

				// System.out.println("\tangle = " + Math.toDegrees(angle));

				if (angleBetweenSelectedNodes == Double.MAX_VALUE
						|| Math.abs(180 - Math
								.toDegrees(angleBetweenSelectedNodes)) > Math
								.abs(180 - Math.toDegrees(angle))) {
					selectedNode1 = node1;
					selectedNode2 = node2;
	...
	public static double angleBetween2Lines(double l1x1, double l1y1,
	...
	public static double kmToProjectionPts(double km, Node refNode1,
			Node refNode2) {
		double globalDistKm = distance(Global.minLat, Global.minLon,
				Global.maxLat, Global.maxLon, 'K');
		double globalDistEuc = Math.sqrt(Math.pow(
				Global.maxLat - Global.minLat, 2)
				+ Math.pow(Global.maxLon - Global.minLon, 2));
	...
	// speed is in km/sec
	public static Vector2d getVelocityVector(double angle, double speed,
			Node refNode1, Node refNode2) {
		double globalDistKm = distance(Global.minLat, Global.minLon,
				Global.maxLat, Global.maxLon, 'K');
		// double globalDistProj =
		// Math.sqrt(Math.pow(Global.openStreetMap.projectedMaxX-Global.openStreetMap.projectedMinX,2)+Math.pow(Global.openStreetMap.projectedMaxY-Global.openStreetMap.projectedMinY,2));
		double globalDistEuc = Math.sqrt(Math.pow(
				Global.maxLat - Global.minLat, 2)
				+ Math.pow(Global.maxLon - Global.minLon, 2));
