...
public class IntersectionCrossingBehavior implements VehicleDriverBehavior {
	...
	private static double DISTANCE_BEFORE_REACTING_TO_INTERSECTION = 0.09144; // 300 ft in km
    ...
    // System.out.println("IntersectionCrossingBehavior tick");
      ...
      // System.out.println("i = " + i.id + " getLightForWayOnLane() = " + light);
        ...
        vehicle.setAcceleration(VehicleUtils.determineNecessaryAcceleration(vehicle.speed(), speedLimit, 0.1524));
  	    state = "accel to speed limit";
			}
			// yellow
			else if (light == 1) {
			  System.out.println("yellow light");
			  vehicle.setAcceleration(VehicleUtils.determineNecessaryAcceleration(vehicle.speed(), 20, d2I));
    	    ...
    	    state = "maintaining speed, far from light";
  			  ...
  			  if (d2I < 0.01524)
  			    vehicle.setAcceleration(Vehicle.MAX_NEG_ACCELERATION);
    				...
    				vehicle.setAcceleration(VehicleUtils.determineNecessaryAcceleration(vehicle.speed(), 0, d2I-0.01524));
    	    state = "accel to 0";
  			}
			}

			if (d2I <= Vehicle.DISTANCE_TO_CONSIDER_AS_SAME) {
				Node nextNode = vehicle.navigation.nextNodeOnPath(vehicle.getDestinationNode().id);
